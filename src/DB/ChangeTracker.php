...
class ChangeTracker {
	...
	public function after_save( Table $table, Record $new_record ) {
		...
		foreach ( $table->get_columns() as $column ) {
			...
			$col_name = $column->get_name() . Record::FKTITLE;
			//var_dump("callable? $col_name", get_class($this->old_record), is_callable( array( $this->old_record, $col_name ) ) );
			//$old_val = ( is_callable( array( $this->old_record, $col_name ) ) ) ? $this->old_record->$col_name() : null;
			$old_val = ( $this->old_record ) ? $this->old_record->$col_name() : null;
